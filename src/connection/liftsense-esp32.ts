...
export function parseLiftSenseStatus(body: string): LiftSenseStatus {
  ...
  const distanceMm =
    typeof data.filtered_distance_mm === 'number' ? data.filtered_distance_mm : data.distance_mm
  ...
  if (
    ...
    (data.motor_channel_1_active !== undefined &&
      typeof data.motor_channel_1_active !== 'boolean') ||
    (data.motor_channel_2_active !== undefined &&
  ...
  return {
    ...
    motorChannel1Active:
      typeof data.motor_channel_1_active === 'boolean' ? data.motor_channel_1_active : undefined,
    motorChannel2Active:
      typeof data.motor_channel_2_active === 'boolean' ? data.motor_channel_2_active : undefined,
  }
}
...
export function liftSensePollDelay(
  ...
  return motorActive || nowMs < postStopUntilMs ? ACTIVE_POLL_INTERVAL_MS : idlePollIntervalMs
}

export function liftSenseStatusPath(callbackUrl?: string): string {
  return callbackUrl ? `/v1/status?callback_url=${encodeURIComponent(callbackUrl)}` : '/v1/status'
}
...
export class LiftSenseEsp32 {
  ...
  private async poll(): Promise<void> {
    ...
    try {
      ...
      if (status.motorChannel1Active !== undefined && status.motorChannel2Active !== undefined) {
