...
export class EWeLinkLAN {
  ...
  constructor(log: Logger, debug: boolean, mode: string, ipOverrides: Record<string, string>) {
    this.log = log
    this.debug = debug
    this.mode = mode
  ...
  async getHosts(): Promise<Map<string, LanDeviceInfo>> {
    ...
    if (discovered.length > 0) {
      ...
      this.log.warn(
        '[LAN] No device IPs discovered via mDNS. Devices with manual IP overrides may still work.',
      )
    }
  ...
  private processResponseRecords(response: multicastDns.ResponsePacket): void {
    ...
    for (const record of allRecords) {
      if (record.type === 'PTR' && record.name.includes('_ewelink')) {
        ...
        const deviceId = ptr.replace(/\._ewelink\._tcp\.local\.?$/, '').replace(/^eWeLink_/, '')
  ...
  async startMonitor(): Promise<void> {
    ...
    this.mdns.on('response', (response) => {
      ...
      for (const record of allRecords) {
        ...
        const data = [txt.data1, txt.data2, txt.data3, txt.data4].filter(Boolean).join('')
        ...
        for (const key of PARAMS_TO_KEEP) {
          if (key in decrypted) {
            ;(params as Record<string, unknown>)[key] = (decrypted as Record<string, unknown>)[key]
          }
        }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (code === 'ECONNRESET' && attempt < maxRetries) {
          if (this.debug) {
            this.log.debug(
              '[LAN] ECONNRESET for %s, retry %d/%d',
              deviceId,
              attempt + 1,
              maxRetries,
            )
   ...
   */
  private parseTxtData(data: unknown): Record<string, string> | undefined {
   ...
   */
  private encryptData(params: DeviceParams, lanKey: string): { encryptedData: string; iv: string } {
    const key = createHash('md5').update(Buffer.from(lanKey, 'utf8')).digest()
    const iv = randomBytes(16)
    const cipher = createCipheriv('aes-128-cbc', key, iv)
    const encrypted = Buffer.concat([cipher.update(JSON.stringify(params)), cipher.final()])
    return {
  ...
  private decryptData(data: string, ivBase64: string, lanKey: string): DeviceParams {
    const key = createHash('md5').update(Buffer.from(lanKey, 'utf8')).digest()
    const decipher = createDecipheriv('aes-128-cbc', key, Buffer.from(ivBase64, 'base64'))
