...
if (!deviceIp || !deviceId || !lanKey) {
  console.error('Usage: npx tsx scripts/probe-device.ts <deviceIp> <deviceId> <lanKey>')
  process.exit(1)
}
...
async function sendZeroconf(suffix: string, params: Record<string, unknown>): Promise<unknown> {
  ...
  try {
    const res = await axios.post(`http://${deviceIp}:8081/zeroconf/${suffix}`, payload, {
    ...
    if (axErr.response) {
      console.log(`   HTTP ${axErr.response.status}: ${JSON.stringify(axErr.response.data)}`)
    } else {
...
async function decryptMdns(): Promise<void> {
  ...
  try {
    ...
    console.log(`  IV: ${iv}`)
    console.log(`  Encrypted data (${dataFields.length} fields, ${concatenatedData.length} chars)`)
