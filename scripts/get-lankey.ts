...
async function login(
  ...
  const signature = createHmac('sha256', APP_SECRET).update(JSON.stringify(data)).digest('base64')

  const res = await axios.post<LoginResponse>(`https://${httpHost}/v2/user/login`, data, {
    headers: {
  ...
  if (body.error === 10004 && body.data?.region) {
    ...
    const newHost = region === 'cn' ? 'cn-apia.coolkit.cn' : `${region}-apia.coolkit.cc`
  ...
  if (body.error !== 0 || !body.data?.at) {
    throw new Error(`Login failed: ${body.msg || JSON.stringify(body)} (error ${body.error})`)
  }
...
async function getHomes(
  ...
  const res = await axios.get<FamilyResponse>(`https://${httpHost}/v2/family`, {
    headers: {
...
async function getDevices(
  ...
  const res = await axios.get<DeviceResponse>(`https://${httpHost}/v2/device/thing`, {
    headers: {
      Authorization: `Bearer ${aToken}`,
      'Content-Type': 'application/json',
      'X-CK-Appid': APP_ID,
...
async function main() {
  ...
  if (!email) {
    console.error('Usage: npx tsx scripts/get-lankey.ts <email> [countryCode] [region]')
    console.error('  Example: npx tsx scripts/get-lankey.ts user@email.com "+1" us')
    process.exit(1)
  }
  ...
  const httpHost = region === 'cn' ? 'cn-apia.coolkit.cn' : `${region}-apia.coolkit.cc`
  ...
  for (const home of homes) {
    ...
    const devices = things.filter((t) => t.itemType === 1 || t.itemType === 2)
    if (devices.length === 0) {
